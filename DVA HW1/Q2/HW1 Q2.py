...
class HW2_sql():
    ...
    def part_c(self,connection):
        ...
        part_c_sql = '''
        SELECT printf('%.2f',COUNT(id)/(select count(id)*1.0 from movies)) FROM movies WHERE score >= 7 AND score <= 20'''
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = '''
        select distinct movie_title, score,cast_count from
        (select distinct a.title as movie_title,printf('%.2f',a.score) as score, printf('%.2f',count(b.cast_id)) as cast_count
